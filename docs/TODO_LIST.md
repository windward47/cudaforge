...
当前 32×32 warp-tiled 实现对 sm_86 远未达到峰值性能。RTX 2050 (sm_86) 完整支持 `mma.sync` 指令（FP16→FP32）。

**修复方向**:
1. 在 matmul dispatch 中增加 `K >= 1024` 且 M,N 对齐的判断
2. 使用 `nvcuda::wmma` API 实现 `matmul_f32_tc` kernel
3. 或在数据量小时增加基于 `cublas` 的可选 fast path

### M4. conv_cuda 硬编码魔数改为设备属性查询

**文件**: `src/operator/nn/conv_cuda.cu:247`

`smem_bytes <= 8192` 硬编码了 8KB 共享内存阈值。不同 GPU 的最大共享内存不同（sm_86 支持可配置到 100KB）。

**修复方向**: 使用 `cudaDeviceGetAttribute` 查询 `cudaDevAttrMaxSharedMemoryPerBlock` 动态决策。
...
**文件**: `src/application/engine/graph.c:244-370`

ReLU/Sigmoid/GELU 的每个 kernel launch 都有 ~5-10μs 固定开销。相邻的 Conv→BN→ReLU 应合并为单个 kernel。

**修复方向**: 在 `graph_execute` 中增加相邻元素级算子的融合逻辑，生成融合 kernel 或链式调用。
...
| 状态 | 数量 | 清单 |
| --- | --- | --- |
| 已完成 | 28 | C1-C8, H1-H7, M1, M2, M5, M7-M12, L1, L5, 以及 ONNX external_data 加载修复 + 真实 MNIST CNN 端到端推理验证 |
| 进行中 | 0 | — |
| 待修复 | 10 | M3, M4, M6, L2-L4, L6-L9 |

> **最后更新**: 2026-05-25。第四轮: ONNX external_data 权重加载修复，真实 MNIST CNN 模型端到端推理验证通过（14/14 测试），文档更新。

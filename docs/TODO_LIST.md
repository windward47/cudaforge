...
为支持 YOLOv8n 端到端 CUDA 推理，补齐 5 个缺失算子：

| 算子 | ONNX Op | 新增文件 |
| --- | --- | --- |
| SiLU | `SiLU` | 融入 activations.c / activations_cuda.cu |
| Mul | `Mul` | mul_int.h, mul.c, mul_cuda.cu, test_mul.c |
| Concat | `Concat` | concat_int.h, concat.c, concat_cuda.cu, test_concat.c |
| Resize | `Resize` | resize_int.h, resize.c, resize_cuda.cu, test_resize.c |
| Transpose | `Transpose` | transpose_int.h, transpose.c, transpose_cuda.cu, test_transpose.c |

基础设施变更：graph.h enum 扩展、graph.c op_name()/SiLU 处理、onnx_loader.c 5 个 op 映射+形状推导+参数构建、operator_init.c 8 个注册函数、CMakeLists.txt 新源文件及测试目标。YOLOv8n 端到端测试框架就绪（gen_yolo_test.py + test_yolo.c），待模型文件生成后执行实际推理。

## 进度

| 状态 | 数量 | 清单 |
| --- | --- | --- |
| 已完成 | 41 | C1-C11, H1-H7, M1-M12, L1-L9, 以及 ONNX external_data 加载修复 + ResNet-18 端到端 CUDA 推理验证 + MatMul Tensor Core + Conv 设备属性查询 + Kernel Fusion + YOLOv8n 5 算子补齐 |
| 进行中 | 0 | — |
| 待修复 | 0 | — |

> **最后更新**: 2026-05-26。第八轮：YOLOv8n 5 个缺失算子实现。CPU + CUDA kernel 均已通过单元测试，compute-sanitizer 零错误。YOLOv8n test 框架就绪（gen_yolo_test.py + test_yolo.c），待生成 ONNX 模型文件后跑端到端推理。

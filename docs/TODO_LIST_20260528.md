...
**实际成果 (2026-05-28)**:
- MHA fused kernel: `grid=(B*S)`, 每 block 处理一个 query position，内部循环所有 head
- CPU 回退验证: vs unfused ref max_diff=3.81e-06
- CUDA 验证: vs CPU max_diff=7.63e-06（小尺寸），BERT-base 相对误差 1.27e-06
- compute-sanitizer: 0 errors
- BERT-base benchmark: CPU 44.76 ms, CUDA 30.35 ms (1.47× speedup for naive kernel)
- 全测试套件: 31/31 通过，0 regression
- 融合 pass: 22 节点 → 1 MHA_Fused + 21 skipped，非破坏性保存/恢复机制

**关键修复 (本阶段)**:
1. CUDA kernel `smem_scores` 共享内存竞态 → 改为寄存器数组
2. Kernel 架构: `grid=(B*H)` 多 block 写同一输出 → 改为 `grid=(B*S)` 单 block 写唯一输出
3. Test 修复: Unity 未初始化 → 自定义 CHECK 宏; missing memcpy; wrong set_input indices
...
| 状态 | 数量 | 内容 |
|------|------|------|
| 已完成 | 20 | R1-R3（可靠性）、Q1-Q4（质量）、P1-P3（性能）、A1-A4（BERT Phase A）、B1-B5（BERT Phase B 完整）、**C1-C2（MHA 融合优化）** |
| 待开始 | 0 | 全部完成 |
...
> **最后更新**: 2026-05-28。第十三轮: Phase B (BERT 完整推理算子) 全部完成; 第十四轮: **Phase C (MHA 融合优化) 全部完成**。CPU+CUDA 双后端验证通过，compute-sanitizer 零错误。**31/31 测试全绿**。

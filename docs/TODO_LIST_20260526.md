...
基础设施变更：graph.h enum 扩展、graph.c op_name()/SiLU 处理、onnx_loader.c 5 个 op 映射+形状推导+参数构建、operator_init.c 8 个注册函数、CMakeLists.txt 新源文件及测试目标。

**2026-05-26 端到端验证结果**:
- **CPU vs PyTorch**: max_diff = 7.93e-04, 0 mismatches (>1e-2 阈值)
- **CUDA vs PyTorch**: max_diff = 8.54e-04, 0 mismatches (>1e-2 阈值)
- **compute-sanitizer**: 0 errors
- 模型: YOLOv8n (1×3×640×640 → 1×84×8400), 234 算子节点, 14 种 op 类型

验证过程中发现并修复了以下问题：
1. **Concat 负轴归一化**: YOLO detection head 使用 axis=-1 (3D tensor)，需在形状推导和参数构建中归一化为 `ndim + axis`
2. **Concat 通用化**: 原 kernel 仅支持 4D NCHW，重写为 outer/inner 通用算法支持任意 ndim/axis
3. **Sub/Div 不可交换性**: 初始值设定项作为第一输入时会被错误移到 weights 数组末尾，修复为对 Sub/Div 保留输入顺序
4. **Kernel fusion 多消费者 bug**: Conv→Sigmoid 融合未检查 Conv 输出是否被其他节点消费。YOLOv8n 的 SiLU 模式 (Conv→Sigmoid→Mul) 中 Conv 输出同时被 Sigmoid 和 Mul 消费，融合导致 Mul 读到未写入的陈旧数据。修复：融合前扫描所有节点计数消费者，>1 个消费者时跳过融合

## 进度

| 状态 | 数量 | 清单 |
| --- | --- | --- |
| 已完成 | 41 | C1-C11, H1-H7, M1-M12, L1-L9, 以及 ONNX external_data 加载修复 + ResNet-18 端到端 CUDA 推理验证 + MatMul Tensor Core + Conv 设备属性查询 + Kernel Fusion + YOLOv8n 5 算子补齐 + 端到端验证 |
| 进行中 | 0 | — |
| 待修复 | 0 | — |

> **最后更新**: 2026-05-26。第九轮：YOLOv8n 端到端 CUDA 推理验证通过。修复 Concat 通用化、Sub/Div 输入顺序、kernel fusion 多消费者 bug。CPU + CUDA 均与 PyTorch 参考一致，compute-sanitizer 零错误。

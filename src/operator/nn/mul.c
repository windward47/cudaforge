...
static void mul_avx2_scalar(const float* a, float b_scalar, float* out, int64_t n) {
...
/* 逐元素: out[i] = a[i] * b[i] */
static void mul_avx2_vec(const float* a, const float* b, float* out, int64_t n) {
    int64_t i = 0;
    for (; i + 8 <= n; i += 8) {
        __m256 va = _mm256_loadu_ps(a + i);
...
#if defined(USE_AVX2)
            mul_avx2_vec(a_blk, b, out_blk, BN);
#else

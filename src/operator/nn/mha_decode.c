/* CPU reference for MHA decode — single-token attention with KV-cache.
   inputs:  [X_new(B,1,D), K_cache(B,max_seq,H,d), V_cache(B,max_seq,H,d),
...
int mha_decode_f32(const void* inputs[], void* outputs[],
    ...
    const float* K_cache = (const float*)inputs[1];    /* (B, max_seq, H, d) */
    const float* V_cache = (const float*)inputs[2];    /* (B, max_seq, H, d) */
    ...
    memcpy(K_cache_out, K_cache, (size_t)B * max_seq * H * d * sizeof(float));
    memcpy(V_cache_out, V_cache, (size_t)B * max_seq * H * d * sizeof(float));
    ...
    for (int64_t b = 0; b < B; b++) {
        ...
        for (int64_t h = 0; h < H; h++) {
            ...
            /* K_new = x · WK + bK, V_new = x · WV + bV → write to cache at cache_len */
            float K_new[64], V_new[64];
            for (int64_t di = 0; di < d; di++) {
                float k_acc = 0.0f, v_acc = 0.0f;
                for (int64_t j = 0; j < D; j++) {
                    k_acc += x[j] * WK[j * D + ho + di];
                    v_acc += x[j] * WV[j * D + ho + di];
                }
                K_new[di] = k_acc + (bK ? bK[ho + di] : 0.0f);
                V_new[di] = v_acc + (bV ? bV[ho + di] : 0.0f);
            }
            /* Write to cache */
            int64_t cache_idx = (b * max_seq + cache_len) * H * d + ho;
            for (int64_t di = 0; di < d; di++) {
            ...
            for (int64_t t = 0; t < total_len; t++) {
                float dot = 0.0f;
                int64_t k_off = (b * max_seq + t) * H * d + ho;
            ...
            for (int64_t t = 0; t < total_len; t++) {
                ...
                int64_t v_off = (b * max_seq + t) * H * d + ho;

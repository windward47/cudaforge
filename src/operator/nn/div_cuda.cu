...
__global__ void div_f32_no_broadcast_kernel(const float* a, const float* b,
                                              float* out, int64_t N) {
    int64_t i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < N) out[i] = a[i] / b[i];
}

__global__ void div_f32_broadcast_c_kernel(const float* a, const float* b,
...
int div_f32_cuda(const void* inputs[], void* outputs[],
...
extern "C" int register_div_f32_cuda(void) {
    static operator_registry_t reg = {
        .name = "div_f32_cuda", .data_type = "f32",
        .func = div_f32_cuda, .version = 1, .flags = OP_FLAG_NONE,
    };

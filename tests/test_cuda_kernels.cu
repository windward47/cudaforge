...
void test_relu_f32_cuda(void) {
    ...
    float h_out[8] = {0};
    float *d_in, *d_out;
    if (cudaMalloc(&d_in, n * sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&d_out, n * sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(d_in, h_in, n * sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    const void* inputs[]  = {d_in, &n};
    void*       outputs[] = {d_out};
    if (op->func(inputs, outputs, NULL, NULL) != 0) TEST_FAIL();
    if (cudaMemcpy(h_out, d_out, n * sizeof(float), cudaMemcpyDeviceToHost) != cudaSuccess) TEST_FAIL();
    if (cudaDeviceSynchronize() != cudaSuccess) TEST_FAIL();
...
void test_matmul_f32_cuda(void) {
    ...
    float *d_A, *d_B, *d_C;
    if (cudaMalloc(&d_A, M*K*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&d_B, K*N*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&d_C, M*N*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(d_A, h_A, M*K*sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(d_B, h_B, K*N*sizeof(float), cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    ...
    if (cudaMemcpy(h_C, d_C, M*N*sizeof(float), cudaMemcpyDeviceToHost) != cudaSuccess) TEST_FAIL();
    if (cudaDeviceSynchronize() != cudaSuccess) TEST_FAIL();
...
void test_conv2d_f32_cuda(void) {
    ...
    if (cudaMalloc(&d_in,C*H*W*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&d_w,K*C*KH*KW*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&d_out,K*OH*OW*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(d_in,h_in,C*H*W*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(d_w,h_w,K*C*KH*KW*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    ...
    if (cudaMemcpy(gpu_out,d_out,K*OH*OW*sizeof(float),cudaMemcpyDeviceToHost) != cudaSuccess) TEST_FAIL();
    if (cudaDeviceSynchronize() != cudaSuccess) TEST_FAIL();
...
void test_maxpool2d_f32_cuda(void) {
    ...
    float *d_in,*d_out;
    if (cudaMalloc(&d_in,C*H*W*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&d_out,C*OH*OW*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(d_in,h_in,C*H*W*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    ...
    if (cudaMemcpy(h_out,d_out,C*OH*OW*sizeof(float),cudaMemcpyDeviceToHost) != cudaSuccess) TEST_FAIL();
    if (cudaDeviceSynchronize() != cudaSuccess) TEST_FAIL();
...
void test_batchnorm_f32_cuda(void) {
    ...
    if (cudaMalloc(&dx,C*hw*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&dg,C*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&db,C*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&dm,C*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&dv,C*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&dd,C*hw*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(dx,x,C*hw*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(dg,g,C*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(db,b,C*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(dm,m,C*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(dv,v,C*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    ...
    if (cudaMemcpy(o,dd,C*hw*sizeof(float),cudaMemcpyDeviceToHost) != cudaSuccess) TEST_FAIL();
    if (cudaDeviceSynchronize() != cudaSuccess) TEST_FAIL();
...
void test_sigmoid_f32_cuda(void) {
    ...
    float *di,*dd;
    if (cudaMalloc(&di,n*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMalloc(&dd,n*sizeof(float)) != cudaSuccess) TEST_FAIL();
    if (cudaMemcpy(di,hi,n*sizeof(float),cudaMemcpyHostToDevice) != cudaSuccess) TEST_FAIL();
    const void* inputs[]={di,&n}; void* outputs[]={dd};
    if (op->func(inputs,outputs,NULL,NULL) != 0) TEST_FAIL();
    if (cudaMemcpy(ho,dd,n*sizeof(float),cudaMemcpyDeviceToHost) != cudaSuccess) TEST_FAIL();
    if (cudaDeviceSynchronize() != cudaSuccess) TEST_FAIL();

...
/* FP16 Tensor Core MHA test — calls mha_fused_f16_cuda directly */
static void test_mha_fused_f16(void) {
#ifdef USE_CUDA
    fprintf(stderr, "\n=== MHA Fused FP16 CUDA Test ===\n");

    const operator_registry_t* fp16_reg = operator_find("mha_fused_f16_cuda");
    if (!fp16_reg || !fp16_reg->func) {
        fprintf(stderr, "SKIP: mha_fused_f16_cuda not registered\n");
        return;
    }

    /* Use same dimensions as the CUDA test (macros B, S, H, d, D) */
    int64_t shape3[] = {B, S, D};
    int64_t shapeW[] = {D, D};
    int64_t shapeb[] = {D};

    tensor_t* tX = tensor_create(DATA_TYPE_F32, 3, shape3);
    tensor_t* tR = tensor_create(DATA_TYPE_F32, 3, shape3);
    tensor_t* tY = tensor_create(DATA_TYPE_F32, 3, shape3);
    tensor_t* twQ = tensor_create(DATA_TYPE_F32, 2, shapeW);
    tensor_t* tbQ = tensor_create(DATA_TYPE_F32, 1, shapeb);
    tensor_t* twK = tensor_create(DATA_TYPE_F32, 2, shapeW);
    tensor_t* tbK = tensor_create(DATA_TYPE_F32, 1, shapeb);
    tensor_t* twV = tensor_create(DATA_TYPE_F32, 2, shapeW);
    tensor_t* tbV = tensor_create(DATA_TYPE_F32, 1, shapeb);
    tensor_t* twO = tensor_create(DATA_TYPE_F32, 2, shapeW);
    tensor_t* tbO = tensor_create(DATA_TYPE_F32, 1, shapeb);

    /* Fill with deterministic random data */
    random_fill(tX->data, B*S*D, 42);
    random_fill(tR->data, B*S*D, 99);
    random_fill(twQ->data, D*D, 123);
    random_fill(tbQ->data, D, 456);
    random_fill(twK->data, D*D, 789);
    random_fill(tbK->data, D, 234);
    random_fill(twV->data, D*D, 567);
    random_fill(tbV->data, D, 890);
    random_fill(twO->data, D*D, 345);
    random_fill(tbO->data, D, 678);

    float scale = 1.0f / sqrtf((float)d);
    mha_fused_params_t params;
    memset(&params, 0, sizeof(params));
    params.batch_size = B; params.seq_len = S;
    params.hidden_size = D; params.num_heads = H;
    params.head_dim = d; params.scale = scale;
    params.has_residual = true;
    ...
    void* outputs[] = { tY->data_device };

    int ret = fp16_reg->func(inputs, outputs, (const operator_params_t*)&params, NULL);
    if (ret != 0) {
        fprintf(stderr, "FAIL: mha_fused_f16_cuda returned %d\n", ret);
        goto cleanup_f16;
    }

    tensor_copy_to_host(tY);

    /* Compare with CPU reference */
    {
        inference_graph_t* g = graph_create();
        tensor_t* tY_cpu = tensor_create(DATA_TYPE_F32, 3, shape3);
        int tidX  = graph_add_tensor(g, tX);
        int tidR  = graph_add_tensor(g, tR);
        int tidY  = graph_add_tensor(g, tY_cpu);
        graph_add_tensor(g, twQ); graph_add_tensor(g, tbQ);
        graph_add_tensor(g, twK); graph_add_tensor(g, tbK);
        graph_add_tensor(g, twV); graph_add_tensor(g, tbV);
        graph_add_tensor(g, twO); graph_add_tensor(g, tbO);
        ...
        tensor_t* inputs_cpu[]  = {tX};
        tensor_t* outputs_cpu[] = {tY_cpu};
        graph_execute(g, inputs_cpu, outputs_cpu, false);

        /* Compare */
        float max_abs = 0.0f, max_rel = 0.0f;
        int has_nan = 0;
        int64_t n_elem = B * S * D;
        const float* Y_fp16 = (const float*)tY->data;
        const float* Y_cpu  = (const float*)tY_cpu->data;
        for (int64_t i = 0; i < n_elem; i++) {
            if (Y_fp16[i] != Y_fp16[i]) { has_nan = 1; break; }  /* NaN check */
            float diff = fabsf(Y_fp16[i] - Y_cpu[i]);
            float cpu_abs = fabsf(Y_cpu[i]);
            if (diff > max_abs) max_abs = diff;
            float rel = cpu_abs > 1e-8f ? diff / cpu_abs : diff;
            if (rel > max_rel) max_rel = rel;
        }
        fprintf(stderr, "FP16 vs CPU: max_abs=%.2e  max_rel=%.2e  has_nan=%d\n", max_abs, max_rel, has_nan);
        if (has_nan) {
            fprintf(stderr, "FAIL: FP16 output contains NaN\n");
        } else if (max_rel > 1e-2f) {
            fprintf(stderr, "FAIL: FP16 max_rel=%.2e > 1e-2\n", max_rel);
        } else {
            fprintf(stderr, "MHA Fused FP16 CUDA: PASS (max_rel=%.2e)\n", max_rel);
        }
        graph_destroy(g);
    }

cleanup_f16:
    tensor_destroy(tX); tensor_destroy(tR); tensor_destroy(tY);
